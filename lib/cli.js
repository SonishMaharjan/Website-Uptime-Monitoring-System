...
cli.responders.stats = function () {
  ...
  //it memory of process
  // console.log(v8.getHeapStatistics());
  // console.log("----");

  // const used = process.memoryUsage();
  // for (let key in used) {
  //   console.log(`${key} ${used[key]} MB`);
  // }
    ...
    if (!err && checkIds && checkIds.length > 0) {
      ...
      checkIds.forEach(function (checkId) {
        _data.read("checks", checkId, function (err, checkData) {
          ...
          //if the user has specified the state, or hsnt specified any state, include the current check accordingly
          if (
...
cli.responders.listLogs = function () {
  ...
  //stdout/stderr are readabli in child process unlike other process whre it is writable
  //when data arrive in stdout it is read and 'data' event is called and is logged to main process
...
cli.responders.moreLogInfo = function (str) {
  //get the logFilename from string
  //Get the Id from the string
  var arr = str.split("--");
  var logFileName =
    typeof arr[1] == "string" && arr[1].trim().length > 0
