...
//initiall _performance was used.. just felt lazy to change all
...
handlers._tokens.post = function (data, callback) {
  ...
  var password =
    ...
    data.payload.password.trim().length > 0
      ...
      : null;
  //Activate observer
  //define this befor measure
  const obs = new PerformanceObserver((list) => {
    //does not show anythins in list ** maybe because it is non enumuerable propertis
    // console.log(list);

    //but below code return value
    // console.log(list.getEntries());
    const entry = list.getEntries()[0];
    debug("\x1b[33m%s\x1b[0m", `Time for ('${entry.name}'): ${entry.duration}`);
  });
  obs.observe({ entryTypes: ["measure"], buffered: false });

  _performance.mark("inputs validated");
  if (phone && password) {
    _performance.mark("beginign user lookup");
      ...
      if (!err && userData) {
        ...
        if (hashedPassword == userData.hashedPassword) {
          ...
          _data.create("tokens", tokenId, tokenObject, function (err) {
            ...
            //gethering all the measuremnts
            //it also pritns using above PerformanceObserver
            ...
            //Logout al measurements
            // var measurements = _performance.getEntries();

            // measurements.forEach(function (measurement) {
            //   debug("\x1b[33m%s\x1b[0m", measurement);
            // });
